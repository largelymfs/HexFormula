...
/**
 * fix the css style of the formula html code and return the fixed html code.
 * @parameter formulaHtml original formula html code string
 * @return fixed html code string
 */
var fontSize = "18px";
function fixStyle(formulaHtml) {
    if(formulaHtml === undefined || formulaHtml.trim() == "")
        return formulaHtml;

    var jqObj = $(formulaHtml.trim());
    jqObj.find("table").each(function(){
        $(this).css({
    ...
    jqObj.find("table").each(fixBorder);
    jqObj.find("table td").each(fixBorder);
    var html = jqObj.html().replace(new RegExp("b.png","gm"), "_1.png");
    return "<span style='font-family:\"Century Schoolbook\";font-style:italic;font-size:" + fontSize + "'>" + html + "</span>";
}
...
function fixBorder() {
    ...
    jqObj.css({margin:"0", padding:"0"});
    var styles = attr.split(";");
    var props = $.map(styles, function(item){
        return item.split(":")[0].trim();
    });
    if($.inArray("border", props) >= 0) //already defined border style, ignore
        return;
    if($.inArray("border-top", props) < 0){ //border-top not defined, fix it
        jqObj.css("border-top", "0");
    }
    if($.inArray("border-right", props) < 0){ //border-right not defined, fix it
        jqObj.css("border-right", "0");
    }
    if($.inArray("border-bottom", props) < 0){ //border-bottom not defined, fix it
        jqObj.css("border-bottom", "0");
    }
    if($.inArray("border-left", props) < 0){ //border-left not defined, fix it
        jqObj.css("border-left", "0");
    }
}
...
$(function () {
    $("#submit").click(function () {
        ...
        // window.close();
        newstr = fixStyle(newstr);
        var editor = parent.tinyMCE.activeEditor;
        var html = editor.getContent({format: 'raw'});
        //write back to the tinyMCE editor and append a empty paragraph behind the formula
        editor.setContent(html + newstr + "<p><br/></p>", {format: 'raw'});
        //parent.document.all["test"].innerHTML = newstr;
        $(parent.document.getElementById("lean_overlay")).click();
        $("#reset").click();
    });
